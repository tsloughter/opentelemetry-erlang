...
-define(SPAN_CTX, ot_span_ctx_key).
...
start_span(Name, Opts) ->
    case ot_ctx_pdict:get(?SPAN_CTX) of
        {SpanCtx, _}=Ctx ->
            SpanCtx1 = ot_span_ets:start_span(Name, Opts#{parent => SpanCtx}),
            ot_ctx_pdict:with_value(?SPAN_CTX, {SpanCtx1, Ctx}),
            SpanCtx1;
         _ ->
            SpanCtx = ot_span_ets:start_span(Name, Opts#{parent => undefined}),
            ot_ctx_pdict:with_value(?SPAN_CTX, {SpanCtx, undefined}),
            SpanCtx
    end.
...
with_span(SpanCtx, Fun) ->
    ot_ctx_pdict:with_value(?SPAN_CTX, {SpanCtx, undefined}, Fun).
...
current_ctx() ->
    ot_ctx_pdict:get(?SPAN_CTX).

%%--------------------------------------------------------------------
%% @doc
%% Finishes the span in the current pdict context. And sets the parent
%% as the current span ctx.
%% @end
%%--------------------------------------------------------------------
-spec finish() -> ok.
finish() ->
    {SpanCtx, ParentCtx} = current_ctx(),
    ot_span_ets:finish_span(SpanCtx),
    ot_ctx_pdict:with_value(?SPAN_CTX, ParentCtx),
    ok.
