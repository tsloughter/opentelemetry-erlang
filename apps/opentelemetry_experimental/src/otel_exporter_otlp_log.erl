...
init(Opts) ->
    case otel_exporter_otlp:init(Opts, opentelemetry_logs_service) of
...
export({Logs, Config}, Resource,
       #state{protocol=http_protobuf, transport=Transport}) ->
    case otel_otlp_logs:to_proto(Logs, Resource, Config) of
        empty ->
            ok;
        ProtoMap ->
            Body = opentelemetry_exporter_logs_service_pb:encode_msg(
                     ProtoMap, export_logs_service_request),
            otel_exporter_otlp:export(Body, Transport)
    end;
export({Logs, Config}, Resource,
       #state{protocol=grpc, transport=Transport}) ->
    case otel_otlp_logs:to_proto(Logs, Resource, Config) of
        empty ->
            ok;
        Request ->
