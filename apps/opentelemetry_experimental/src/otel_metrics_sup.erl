...
init([Opts]) ->
    ...
    MetricEtsSup = #{id => otel_metrics_ets_sup,
                     start => {otel_metrics_ets_sup, start_link, [Opts]},
                     restart => permanent,
                     shutdown => 5000,
                     type => worker,
                     modules => [otel_metrics_ets_sup]},
    ...
    ChildSpecs = [MetricEtsSup, MeterServer, MetricReaderSup],
